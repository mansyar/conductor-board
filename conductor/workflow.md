...
3.  **Test-Driven Development:** Write unit tests before implementing
    functionality (logic-bearing code only — see Testing Scope below)
4.  **High Code Coverage:** Aim for >80% code coverage for logic-bearing code
...
**Testing applies only to logic-bearing code** — functions, modules, and
components that contain non-trivial logic (business rules, parsing, state
transitions, data transformations, phase detection, etc.).

The following are **exempt** from mandatory tests and from coverage targets:

-   Pure presentation/JSX layout without logic
-   Trivial pass-through adapters and thin wrappers
-   Static configuration and generated scaffolding

When in doubt, ask: *does this code make a decision?* If it does, test it. This
scope governs the TDD red/green phases, the >80% coverage target, and the
phase-completion test-verification protocol below.
